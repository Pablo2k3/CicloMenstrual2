...
object CycleRules {
    fun validateStart(start: Long, now: Long): CycleValidationError? =
        CycleValidationError.START_IN_FUTURE.takeIf { start > DateNormalizer.todayKey(now) }
    ...
    fun validateEnd(
        ...
        end > DateNormalizer.todayKey(now) -> CycleValidationError.END_IN_FUTURE
    ...
    fun isOngoing(cycle: Cycle, cycles: List<Cycle>, now: Long): Boolean =
        cycle.endDate == null &&
            ...
            DateNormalizer.daysBetween(cycle.startDate, DateNormalizer.todayKey(now)) < 7
}
